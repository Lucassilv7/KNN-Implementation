import numpy as np
from knn import minkowski_distance
...
class KDimensionalTree:
    ...
    def __init__(self, X, leaf_size=30, metric='minkowski', p=2):
        """
    ...
    def build_tree(self, indices, depth):
        ...
        return KDNode(indices=sorted_indices, split_dim=axis, split_value=split_value, left=left_indices, right=right_indices)
    ...
    def _query_point(self, node, x, k, heap):
        if node.indices is not None:
            for idx in node.indices:
                dist = minkowski_distance(x, self.X[idx], p=self.p)
